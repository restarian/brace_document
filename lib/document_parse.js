...
define(["bracket_print", "bracket_utils", "./sort"], function(print, utils, sort) {
	...
	var def = function(up, cb, err) {
		...
		if ( up instanceof print ) 
			this.up = up = up.spawn(up.log_title+" -> document_parse") 
		else {
			if ( typeof up === "function" )
				cb = up
			this.up = up = print({title_stamp: false}, "document_parse")
		}
		...
		this.url = this.reverse_sort = this.sort = this.relative_backup_dir = this.relative_docs_dir = this.project_root = this.origin_url = this._origin_url = this.branch = ""
	...
	def.prototype = { 
		...
		find_git_root: function(dir, cb, err) {
			...
			utils.Spawn("git", ["rev-parse", "--show-toplevel"], {cwd: dir}, (code, stdout, stderr) => {
				...
				if ( stderr )
					return err(this.up_err.log("find_git_root()", stderr, "->", dir))
				...
				cb(this.project_root)

			}, err)
	
		},
		find_relative_data: function(dir, cb, err) {
			...
			utils.Spawn("git", ["config", "--get", "remote.origin.url"], {cwd: dir}, (code, stdout, stderr) => { 
				...
				if ( stderr )
					return err(this.up_err.log("find_git_origin()", stderr, "->", this.project_root))
				
				this._origin_url = url.parse(stdout)

				// The current branch is used so that the other branches commit separate docs.
				utils.Spawn("git", ["branch", "-q"], (code, stdout, stderr) => {

					if ( stderr )
						return err(this.up_err.log("find_git_origin()", stderr))

					this.branch = stdout.replace(/\s*\*\s*/, "").replace(/[\n\r]*/g, "")
					this.relative_docs_dir = this.relative_backup_dir = path.relative(this.project_root, this.relative_docs_dir)

					if ( this.backup ) 
						this.relative_backup_dir = path.relative(this.project_root, this.backup)

					if ( this.relative_backup_dir.substr(0,2) === ".." || path.parse(this.relative_backup_dir).dir.substr(0,2) === ".." )
						return err(this.up_err.s("The directory to use for the documents:", this.relative_backup_dir).l("which resolves to:")
					...
					if ( this.verbose ) 
						this.up.s("Using git repository at:", this.project_root)
							.l("with document files in:").s(this.relative_docs_dir||"./").log()
					if ( this.backup )
						this.up.l("Creating backups at:").s(this.relative_backup_dir).log()

					cb()
					//run_parser(path.join(base_dir, "/", this.relative_docs_dir), link_url, relative_backup_dir, cb, err)

				}, this.up_err.log)
			}, this.up_err.log)
		},
		sort_list: function(list, cb, err) {
			...
			if ( typeof sort[qualifier] !== "function" )
				this.up_err.log("Sorting type", qualifier, "is not available in the sort module.")
			else {
				if ( this.sort === "depth" ) {
					list = list.sort(sort["alphanumeric"])
					list = list.sort(sort[(this.reverse_sort&&"reverse_"||"")+this.sort])
				}
				else
				...
				data.forEach(value => { 
					...
					fs.stat(full_path, (error, stat) => {
						...
						if ( stat.isDirectory() ) { 
							...
							structure.push(a)
							this.aquire_structure(full_path, a[value], internal_callback, err)
						}
			...
			var parse_file = (file_path, base_dir) => {
				...
				file_path.forEach(value => {
					...
					fs.readFile(value, undefined, (error, content) => {
						...
						if ( ! primary_heading )
							this.up.log("A primary heading (#[#,..]), could not be found in document", value)

						var secondary_heading = content.replace(primary_heading[0]).match(this.heading_regex)
						if ( ! secondary_heading )
							this.up.log("A secondary heading (#[#,..]), could not be found in document", value)
						...
						data[value] = { 
							...
							primary_heading: primary_heading && primary_heading[0] || "", 
							secondary_heading: secondary_heading && secondary_heading[0] || "",
							content: content 
						}
				...
				stdout.replace(/(.*)[\n]+/g, function(all, first) {
					...
					if ( plugin.name.match(/brace_document_.*/) ) {
						...
						try {
							...
							var option = module.require(plugin.name+"/option.json")
						} catch(error) {
							err(this.up_err.log(error))
						}
		...
		create_document: function(structure, page_data, backup_dir, cb, err) {
