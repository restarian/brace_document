...
define(["bracket_print", "bracket_utils", "./sort"], function(print, utils, sort) {
	...
	def.prototype = { 
		...
		// Add a directory to a structure object. The directory must be relative (no root identifiers at the begging), and can be in in posix or
		// win32 format. Note: this member is synchronous and returns a value.

			// Set the chaining logging titles to also have this member name. -------------------------------------
			var up_err = this.up_err.spawn(), up = this.up.spawn()
			up.log_title = up.log_title + " - addStructure()"
			up_err.log_title = up.log_title + " - ERROR"
			cb = typeof cb === "function" && cb || function(){}
			// Create the error callback which will transfer the logger from this method into the calling method error callback.
			var err_cb = function() {
			...
			if ( r )
				return err_cb(up_err.log("The directory", directory, "must not contain a root. Found", r, "in as the root."))
			...
			location.forEach(subdir => {
				...
				if ( !list.every(val => { return typeof val === "string" || (subdir in val) }) ) {
					add = {}
					add[subdir] = []
		...
		// most robust API as passable.
			...
			var write_list = (list, relative_dir) => {
				...
				list.forEach(value => {
					...
					up.log("Writing out document", file_path)
					if ( this.option.dryRun ) {
						up.log("Not actually writing out document because the dryRun option is set.")
						if ( !(--cnt) )
							cb()
						return
					}
