...
define(["bracket_print", "bracket_utils", "./sort"], function(print, utils, sort) {
	...
	var def = function(up, cb, err) {
		// The up argument (bracket print instance), argument is optional as one will be created if it is omitted.
		...
		this.project_root = ""
		this.relative_docs_dir = this.option.projectLocation
		this.relative_backup_dir = ""
		this.docs_dir = ""
		this.backup_dir = ""
	...
	def.prototype = { 
			...
			plugin_list.forEach(value => {
				...
				value.option.forEach(opt => {
					...
					if ( "usage" in opt ) {
						...
						var args = [opt.flag, "[" + value.short_name + "] " + opt.help]
						if ( "setter" in opt ) 
		...
		// proper fully qualified paths or resolved relative paths.
			...
			utils.Spawn("git", ["rev-parse", "--show-toplevel"], {cwd: dir}, (code, stdout, stderr) => {
				...
				if ( this.option.backup ) {
					this.backup_dir = path.resolve(this.project_root, this.option.backup)
		...
		sortStructure: function(structure, cb, err) {
			...
			// Set the chaining logging titles to also have this member name. -------------------------------------
			var up_err = this.up_err.spawn(), up = this.up.spawn()
			up.log_title = up.log_title + " - sortStructure()"
			up_err.log_title = up.log_title + " - ERROR"
			cb = typeof cb === "function" && cb || function(){}
			// Create the error callback which will transfer the logger from this method into the calling method error callback.
			var err_cb = function() {
		...
		// both optional. The dir argument will otherwise be set to the docs directory set internally. The structure argument is used internally.
			...
			fs.readdir(path.join(this.project_root, dir), (error, data) => { 
				...
				// This happens when the directory is empty. TODO: remove empty directories from the structure.
				if ( !cnt )
					callback(structure)
		...
		addData: function(data, full_path, base_dir, content, cb, err) {
		// Add the data item to the data object. This member is synchronous.

			// Set the chaining logging titles to also have this member name. -------------------------------------
			var up_err = this.up_err.spawn(), up = this.up.spawn()
			up.log_title = up.log_title + " - addData()"
			up_err.log_title = up.log_title + " - ERROR"
			cb = typeof cb === "function" && cb || function(){}
			// Create the error callback which will transfer the logger from this method into the calling method error callback.
			var err_cb = function() {
			...
			var doc = path.parse(full_path)
			
			var primary_heading = content.match(this.heading_regex)
			if ( ! primary_heading ) {
				up.s("A primary heading ( #+.* ), could not be found within the document", full_path).log()
			...
			if ( ! secondary_heading ) {
				up.s("A secondary heading ( #+.*\n#+.* ), could not be found within the document", full_path).a(". The document filename will be")
			...
			var parse_file = (file_path, base_dir) => {
				...
				file_path.forEach(value => {
					...
					fs.readFile(value, undefined, (error, content) => {
						...
						this.addData(data, value, base_dir, content)

						// After all pages in the structure are collects the callback is called unless the error print instance contains text data.
						if ( !--cnt ) 
							cb(data)

						// The value needs to be bound so that the asynchronous readFile function call gets the correct argument.
					})
				})
			}
		...
		// directory is requested. Otherwise, the originally parsed files will be overwritten with the new content. 
			...
			if ( this.option.dryRun )
				up.log("No actual directories will be created because dry-run option data is set.")
			...
			var tld = path.parse(backup_dir).root
			...
			var make_directory = (dir) => {
				...
				fs.stat(dir, (error, stats) => {
					...
					if ( error ) {
						...
						if ( this.option.dryRun ) {
							up.log("Not actually creating the directory because the dry-run option is set.")
							make_directory(p)
						}
						else
							fs.mkdir(dir, (error) => {
		...
		// listed there will be ran buy this member if it is also contained in the plugin list passed in. The plugin list is not mutated.
			...
			// Set the chaining logging titles to also have this member name. -------------------------------------
			var up_err = this.up_err.spawn(), up = this.up.spawn()
			up.log_title = up.log_title + " - runPlugin()"
			up_err.log_title = up.log_title + " - ERROR"
			cb = typeof cb === "function" && cb || function(){}
			// Create the error callback which will transfer the arguments from this method into the calling method error callback.
			var err_cb = function() {
			...
			if ( mod = plugin_list.pop() ) {
					...
					if ( this.option.enableAll || this.option.pluginEnable.match(RegExp("\,\\s*"+ mod.name +"\\s*\,")) ) {
						var plugin_instance 
						try {
							plugin_instance = mod.plugin(this, null, err_cb) 
						} catch(error) {
		...
		// gathered. E.g. setting plugin_regex to "^mycompany.*" would match the modules mycompany_library and mycompany-scipt.
			...
			var mod_path = module.paths.slice()
			if ( this.option.pluginPath )
				mod_path.unshift(this.option.pluginPath)

			if ( directory ) {
				if ( typeof directory === "string" )
					mod_path.unshift(directory)
				else if ( directory instanceof Array )
					mod_path = directory.concat(mod_path)
			}
			...
			var get_plugin = (dir) => {
				...
				fs.readdir(dir, (error, out) => {
					...
					out.forEach(value => {
						...
						var plugin = path.parse(path.join(dir, value)), mod, option, short_name
						...
						if ( !found[plugin.name] && regex_match ) {
							...
							//short_name = plugin.name.replace(/^(brace|batten|bracket)_document_/, "")
							short_name = plugin.name.replace(RegExp("^"+regex_match[0]), "")
							// This is the plugin object created from a module which gets added to the list and passed into the callback.
							data.push({name: plugin.name, short_name: short_name, option_name: short_name.replace(/\_/g, "-"), 
											dir: plugin.dir, plugin: mod, option: option, package_info: package_info})
						}
					})

					var next_path = mod_path.pop()
					if ( !next_path ) {
			...
			get_plugin(mod_path.pop()||("."+path.sep))
		},
		writeData: function(structure, data, backup_dir, cb, err) {
		...
		// most robust API as passable.
			...
			if ( this.option.dryRun ) {
				up.log("Not writting data to directory/drive because the dry-run option is set.")
				return cb()
			}

			var data, cnt = 0

			Object.keys(data).forEach(function(location, index, proto) {

				page = data[location]
				// The files can be created in any order sense there is no reason why any particular write would fail.
				fs.writeFile(path.join(backup_dir, page.relative_dir, page.file_name), page.content, (error) => { 
