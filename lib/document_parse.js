...
define(["bracket_print"], function(print) {
	...
	def.prototype = { 
		...
		create_structure: function(structure, backup_dir, cb, err) {
			...
			var base = backup_dir.split(path.sep).map(function(value) {
				if ( !value )
					return ""
			...
			// base needs to be before the directories so that it is created first.
			directory = base.concat(directory)
