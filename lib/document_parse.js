...
define(["bracket_print", "bracket_utils", "./sort"], function(print, utils, sort) {
	...
	var def = function(up, cb, err) {
		...
		this.reverse_sort = this.recursive = this.enable_all = false
		this.sort = this.relative_backup_dir = this.relative_docs_dir = this.project_root = this.docs_dir = this.backup_dir = this.plugin_path = ""
		this.plugin_enable = ""
	...
	def.prototype = { 
			...
			plugin_list.forEach((value) => {
				...
				value.option.forEach((opt) => {
					...
					if ( "usage" in opt ) {
						...
						program.usage(program._usage + EOL + EOL + "-- " + title_name + "  " + Array(115-title_name.length).join("-") + EOL + opt.usage)
					}
					else
						program.option(opt.flag, " <- "+ value.short_name + ": "+opt.help)
				})
			})
		...
		findStructure: function(dir, structure, cb, err) {
			...
			else if ( typeof dir === "string" && typeof structure === "function" ) { //&& Object.prototype.toString.call(dir) === "[object Array]" ) {
				err = cb 
				cb = structure 
				structure = [] 
			}
			...
			fs.readdir(path.join(this.project_root, dir), (error, data) => { 
				...
				// This happens when the directory is empty. TODO: remove the empty directory form the structure.
				if ( !cnt )
					callback(structure)
		...
		runPlugin: function(plugin_list, cb, err) {
		...
		// listed there will be ran buy this member if it is also contained in the plugin list passed in. The plugin list is not mutated.
			...
			if ( mod = plugin_list.pop() ) {
					if ( this.enable_all || this.plugin_enable.match(RegExp("\,\\s*"+ aa +"\\s*\,")) ) {
						var plugin_instance 
						try {
							var plugin_instance = mod.plugin(option) 
						} catch(error) {
							up.log("Plugin", mod.name, "does not have a callable constructor.", error)
							return run_plugin()
						}
						if ( !("runThrough" in plugin_instance) ) {
							up.log("Plugin", mod.name, "does not have a runThrough member returne from calling its constructor.")
							run_plugin()
						}
						else {
		...
		// gathered. E.g. setting plugin_regex to "^mycompany.*" would match the modules mycompany_library and mycompany-scipt.
			...
			if ( this.plugin_path )
				mod_path.unshift(this.plugin_path)
			...
			var get_plugin = function(dir) {
				...
				fs.readdir(dir, (error, out) => {
					...
					out.forEach(value => {
						...
						var brace_matched = plugin.name.match(/^brace_document_.*/)
						var regex_matched = this.plugin_regex && plugin.name.match(RegExp(this.plugin_regex)) 
						if ( !found[plugin.name] && ( brace_matched || regex_matched ) ) {
							try {
							...
							short_name = brace_matched && plugin.name.replace(/^brace_document_/, "") || plugin.name
							...
							data.push({name: plugin.name, short_name: short_name, option_name: short_name.replace(/\_/g, "-"), 
											dir: plugin.dir, plugin: mod, option: option})
						}
					})
		...
		writeData: function(structure, data, backup_dir, cb, err) {
			...
			Object.keys(page_data).forEach(function(location, index, proto) {
				...
				fs.writeFile(path.join(backup_dir, data.relative_dir, data.file_name), page.content, (error) => { 
