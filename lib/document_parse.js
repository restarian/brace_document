	...
	def.prototype = { 
		...
		findPath: function(dir, cb, err) {
			...
			utils.Spawn("git", ["rev-parse", "--show-toplevel"], {cwd: dir}, (code, stdout, stderr) => {
				...
				this.relative_docs_dir = this.relative_docs_dir || path.join(".", "/")
				...
				this.relative_backup_dir = this.relative_backup_dir || path.join(".", "/")
		...
		findStructure: function(structure, cb, err) {
			...
			var callback = (function(list) {  
				...
				if ( !list || !list.length )
					up.log("No document pages were found within directory:", this.docs_dir)
				else {

					up.log("Found", structure.length, "documents in:", this.docs_dir)
					if ( this.sort ) 
						this.sortList(list)
				}
			...
			fs.readdir(this.docs_dir, (error, data) => { 
				...
				data.forEach(value => { 
					...
					var full_path = path.join(this.docs_dir, "/", value)
					...
					fs.stat(full_path, (error, stat) => {
						...
						if ( stat.isDirectory() ) { 
							...
							structure.push(a)
							this.findStructure(full_path, a[value], internal_callback, err_cb)
						}
