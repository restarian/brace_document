...
define(["bracket_print"], function(print) {
	...
	def.prototype = { 
		...
		create_all: function(structure, page_data, cb, err) {
			...
			var data, up = this.up, num_pages = 0
			...
			for ( var path in page_data ) {
				...
				var nav_list = this.create_navlink(structure, page_data, path)
		...
		// which have a .md extension.
			...
			var callback = function(l) {  
				...
				if ( ! l || !l.length )
					...
					up.log("Found", list.length, "documents in:", dir)
